...
{ "IF", 7, 4 }, { "EQ", 8, 0 }, { "LT", 9, 0 }, { "GT", 10, 0 }, { "LTEQ", 11, 0 }, { "GTEQ", 12, 0 }, { "PRINT", 13, 2 }, { "READ", 14, 1 } };

void evaluateIntermediateTable(struct IntermediateTable* IntermediateTable);
...
void loadFile()
{
	...
	while (fgets(expression, sizeof(expression), fps))
	{
		...
		while (expression[i] != '\0')
		{
			...
			else if (strcmp(result, "PRINT") == 0)
			{
				...
				{
					intermediateTable[Iindex].parameters[0] = result[0] - 'A';
					intermediateTable[Iindex].parameters[1] = 0;
				}
				else
				{
					int symadd = getSymbolAddress(result, s);
					intermediateTable[Iindex].parameters[0] = symadd;// printf("index=%d\n", Iindex);
					intermediateTable[Iindex].parameters[1] = 1; 
				}
				Iindex++;
			}
	...
	evaluateIntermediateTable(intermediateTable);
...
void evaluateIntermediateTable(struct IntermediateTable* IntermediateTable)
{
	...
	for (; i < Iindex-1;i++)
	{
		if (IntermediateTable[i].op == 1)//MOV(Register to Mem) MOV A,AX
		{
			valuesOfSymbols[IntermediateTable[i].parameters[0]] = registers[IntermediateTable[i].parameters[1]];
		}
		else if (IntermediateTable[i].op == 2)//MOV AX,A
		{
			registers[IntermediateTable[i].parameters[0]] = valuesOfSymbols[IntermediateTable[i].parameters[1]];
		}
		else if (IntermediateTable[i].op == 3)//ADD CX, AX, BX
		{
			registers[IntermediateTable[i].parameters[0]] = registers[IntermediateTable[i].parameters[1]] + registers[IntermediateTable[i].parameters[2]];

		}
		else if (IntermediateTable[i].op == 4)//SUB
		{
			registers[IntermediateTable[i].parameters[0]] = registers[IntermediateTable[i].parameters[1]] - registers[IntermediateTable[i].parameters[2]];
		}

		else if (IntermediateTable[i].op == 5)//MUL
		{
			registers[IntermediateTable[i].parameters[0]] = registers[IntermediateTable[i].parameters[1]] * registers[IntermediateTable[i].parameters[2]];
		}

		else if (IntermediateTable[i].op == 6)//JMP
		{
			i = IntermediateTable[i].parameters[0]-1;
			continue;
		}
		else if(IntermediateTable[i].op== 7) //IF CX EQ DX THEN
		{
			int val1,val2;
			val1 = registers[IntermediateTable[i].parameters[0]];
			val2 = registers[IntermediateTable[i].parameters[2]];

			if (IntermediateTable[i].parameters[1]== 8)//EQ
			{
				if (val1 == val2)
				{
					continue;
					...
					i = IntermediateTable[i].parameters[3]-1;
			}
			else if (IntermediateTable[i].parameters[1] == 9)//LT
			{
				if (val1 < val2)
					continue;
				else
			...
			else if (IntermediateTable[i].parameters[1] == 10)///GT
			{
				if (val1 > val2)
					continue;
				else
			...
			else if (IntermediateTable[i].parameters[1] == 11)///LTEQ
			{
				if (val1 <= val2)
					continue;
				else
			...
			else if (IntermediateTable[i].parameters[1] == 12)///GTEQ
			{
				if (val1 >= val2)
					continue;
				else
		...
		else if (IntermediateTable[i].op==13) //PRINT
		{
			if (IntermediateTable[i].parameters[1] == 0)//print AX
			{
				printf("res is %d \n", registers[IntermediateTable[i].parameters[0]]);
			}
			else //PRINT A
			{
				printf("res is %d\n", valuesOfSymbols[IntermediateTable[i].parameters[0]]);
			}
		}
		else if (IntermediateTable[i].op == 14)//READ
		{
			printf("enter value of %cX",IntermediateTable[i].parameters[0]+'A');
			scanf("%d", &registers[IntermediateTable[i].parameters[0]]);
